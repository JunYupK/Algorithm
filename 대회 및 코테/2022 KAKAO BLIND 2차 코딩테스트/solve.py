...
TOKEN = "403cd601434042db7430978d9a20d109"
...
post_method = lambda url, data: http_method("POST", base_url, url, data=data, token=TOKEN)
put_method = lambda url, data: http_method("PUT", base_url, url, data=data, token=TOKEN)
...
def api_start(problem, init_token):
    assert 1 <= problem <= 2
    resp = http_method("POST", base_url, "/start", data={'problem': problem}, token=init_token, init=True)
...
def solve():
    global TOKEN
    TOKEN = api_start(2, TOKEN)
    num_users = problem_info[2]['num_users']
    ...
    for now in range(595):
        ...
        while i + 1 < len(waiting_line):
            ...
            diff = (skills[waiting_line[i][0]] - skills[waiting_line[i + 1][0]]) / (
                    waiting_line[i][1] // 4 + 1)
            if diff <= 10000:  # matching 성공
